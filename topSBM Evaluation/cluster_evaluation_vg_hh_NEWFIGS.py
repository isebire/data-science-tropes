...
def analyse_clusters(cluster_file, run_name):
    ...
    plt.yticks(np.arange(len(vg_genres_names)), labels=[x for x in vg_genres_names], fontsize=20)
    ...
    plt.xlim(0, 1)
    plt.title('Distribution of Herfindahl-Hirschman Index', fontsize=30)
    plt.xlabel('Herfindahl-Hirschman Index', fontsize=30)
    plt.ylabel('Frequency', fontsize=30)
    plt.xticks(fontsize=20)
    plt.yticks(fontsize=20)
    filename = run_name + '_hh_index_dist.png'
    plt.savefig(filename, bbox_inches='tight')

...
class sbmtm():
    ...
    def topics(self, dict_groups, l=0, n=10):
        '''
    ...
    def topicdist(self, dict_groups, doc_index, l=0):
        # dict_groups =  self.groups[l]
        # dict_groups = self.get_groups(l=l)
    ...
    def clusters(self, dict_groups, l=0,n=10):
        '''
    ...
    def print_topics(self,l=0,format='csv',filename_prefix='',plot_dists=False):
        '''
        Print topics, topic-distributions, and document clusters for a given level in the hierarchy.
        format: csv (default) or html
        NOTE: I EDITED path_save to filename_prefix
        also added plot dists. bc finding the topics in each document is time consuming
        '''
        ...
        ## topics
        print('Topics...')
        dict_topics = self.topics(dict_groups, l=l,n=-1)
        ...
        for t in list_topics:
            list_w = [str(h[0]) + '/' + str(h[1]) for h in dict_topics[t]]
            # edited h[0] -> str(h[0] + '/' + h[1]) ^
            V_t = len(list_w)
            df.iloc[:V_t,t] = list_w
        df=df.dropna(how='all',axis=0)
        if format == 'csv':
            fname_save = 'topsbm_level_%s_topics.csv'%(l)
            filename = filename_prefix + fname_save  # edited
        ...
        topic_dists_for_plot = [[] for i in range(len(list_topics))]

        df = pd.DataFrame(columns=list_columns,index=range(D))
        for i_doc in range(D):
            # This below line is what takes a really long time
            list_topicdist = self.topicdist(dict_groups, i_doc, l=l)
            for topic_number, j in enumerate(list_topicdist):
                topic_dists_for_plot[topic_number].append(j[1])
        ...
        if plot_dists:  # only really meaningful for top 500, also saves time
            print('Plotting the topic distributions for each work')
            plt.figure(figsize=(15,15))
            #ax.subplot(121)
            plt.imshow(topic_dists_for_plot, origin='lower', aspect='auto',
                       interpolation='none', cmap='rainbow')
            plt.xlim(0, len(self.documents))
            plt.ylim(-0.5, len(list_topics) - 0.5)
            # work out how to make it integers
            plt.yticks(range(0, len(list_topics)))
            plt.title('Topic distribution over 500 works with most tropes')
            plt.xlabel('Work (index)')
            plt.ylabel('Topics')
            plt.clim(0, 1)
            plt.colorbar()
            figure_filename = filename_prefix + '_' + str(l) + '_topic_dists.png'
            plt.savefig(figure_filename)

        ## doc-groups
        print('Document clusters...')

        dict_clusters = self.clusters(dict_groups, l=l,n=-1)

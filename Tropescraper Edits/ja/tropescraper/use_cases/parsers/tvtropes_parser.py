...
class TVTropesParser(object):
    FILM_MAIN_SEARCH = 'https://tvtropes.org/pmwiki/pmwiki.php/Main/Anime'
    TROPES_MAIN_SEARCH = 'https://tvtropes.org/pmwiki/pmwiki.php/Main/Tropes'

    BASE_FILM_URL = 'https://tvtropes.org/pmwiki/pmwiki.php/Anime/'
    ...
    MEDIA_TYPES_NOT_TROPES = {'Advertising', 'AdvertisingCampaigns', 'Series', 'AsianAnimation',
                              ...
                              'TelevisionMovieIndex', 'Film', 'Theatre', 'Ballet', 'Opera', 'Vaudeville',
    ...
    def _get_links_from_page(self, page, link_type, only_article=False, remove_link=True, sub_pages=False):
        ...
        if link_type == '/Anime/':
            filtered_links = [link for link in links if (('/Anime/' in link) or ('/Manga/' in link) or ('/LightNovel/' in link)) and 'action' not in link]
        else:

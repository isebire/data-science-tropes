...
data_cleaned = {}
for trope, description in data.items():
    if description != '/':
        data_cleaned[trope] = description

trope_names = list(data_cleaned.keys())
trope_descriptions_laconic = list(data_cleaned.values())

# Compute encodings
trope_embeddings = model.encode(trope_descriptions_laconic, show_progress_bar=True)
...
# Save the embeddings
with open('embeddings_laconic.pkl', 'wb') as f:
...
for trope_id, cluster_id in enumerate(cluster_assignment):
    ...
    clustered_tropes_desc[cluster_id].append(trope_descriptions_laconic[trope_id])
    clustered_tropes[cluster_id].append(trope_names[trope_id])

# Save the results
with open('embeddings_laconic_cluster_agg_MODEL_RESULTS.pkl', 'wb') as f:
    pickle.dump(clustering_model, f)

with open('embeddings_laconic_cluster_agg.pkl', 'wb') as f:
    pickle.dump(clustered_tropes, f)

with open('embeddings_laconic_cluster_agg_descs.pkl', 'wb') as f:
...
for i, cluster in enumerate(clusters):
    ...
    for sentence_id in cluster[0:3]:
        print("\t", trope_descriptions_laconic[sentence_id])
    print("\t", "...")
...
print(len(clustered_tropes))

# Save the results
with open('embeddings_laconic_cluster_fast.pkl', 'wb') as f:
    pickle.dump(clustered_tropes, f)
